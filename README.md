...
| LinkedIn Company Page | `@TuningDigital` | `https://linkedin.com/company/tuningdigital` — footer social-btn (all pages + both engines) + Organization JSON-LD `sameAs` (engines + index.html + about.html). Brand assets in `brand/linkedin/` (logo 400×400, banner 1128×191, SVG sources). **Live with dial mark since 2026-06-08** (replaced the earlier blue-dot upload). |
| X (`@TuningDigital`) Company | `https://x.com/TuningDigital` | Brand assets in `brand/x/` — `profile.png` (400×400, same file as the LinkedIn logo) + `header.png` (1500×500, dark hero with kicker/wordmark/tagline). **Live since 2026-06-08** with the bio line `"Independent reviews of the AI and SaaS tools founders actually use. No sponsored coverage. Editorial rankings never for sale. New reviews Mon + Thu."`. Build script at `/tmp/build_x_assets.py` (not checked in). |
